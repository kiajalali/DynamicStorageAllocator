...
void *mm_malloc(size_t size)
{
  ...
  // edge case
  // better comments and code (tho most of it is the same)
  
  // trying to figure out if free block even has mem space
  ptrFreeBlock = searchFreeList(reqSize);

  // if not enough space
  if (ptrFreeBlock == NULL) {
    // give it more mem pretty much
    requestMoreSpace(reqSize);
    ptrFreeBlock = searchFreeList(reqSize);
    precedingBlockUseTag = TAG_PRECEDING_USED;
  }
  // if we dont need to alloc mroe space just label it as used (ez pz)
  else precedingBlockUseTag = ptrFreeBlock->sizeAndTags & TAG_PRECEDING_USED;

  // findout what the size is then remove from the free list 
  blockSize = SIZE(ptrFreeBlock->sizeAndTags);
  removeFreeBlock(ptrFreeBlock);

  // this just in case the blocksize is bigger than min size, if so u gotta split it in half
  ...
  // if block - size is not bigger its easier
  else {
    *((size_t*) UNSCALED_POINTER_ADD(ptrFreeBlock, (reqSize - WORD_SIZE))) = blockSize;
  ...
  ptrFreeBlock->sizeAndTags |= precedingBlockUseTag;
